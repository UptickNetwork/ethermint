...
// TraceCall implements debug_traceCall: trace execution of a simulated eth_call at a block.
func (k Keeper) TraceCall(c context.Context, req *types.QueryTraceCallRequest) (*types.QueryTraceCallResponse, error) {
	...
	result, _, err := k.traceMessage(ctx, cfg, txConfig, msg, req.TraceConfig, false, tracerConfig)
	if err != nil {
		return nil, err
	}
...
func (k *Keeper) traceTx(
	...
	return k.traceMessage(ctx, cfg, txConfig, *msg, traceConfig, commitMessage, tracerJSONConfig)
}

// traceMessage runs the EVM tracer for an already-built core message.
func (k *Keeper) traceMessage(
	ctx sdk.Context,
	cfg *statedb.EVMConfig,
	txConfig statedb.TxConfig,
	msg core.Message,
	...
	structTracer := logger.NewStructLogger(&logConfig)
	tracer = &tracers.Tracer{
		Hooks:     structTracer.Hooks(),
		GetResult: structTracer.GetResult,
		Stop:      structTracer.Stop,
	}
	...
	res, err := k.ApplyMessageWithConfig(ctx, msg, (*tracing.Hooks)(tracer.Hooks), commitMessage, cfg, txConfig)
	if err != nil {
		return nil, 0, status.Error(codes.Internal, err.Error())
	}
